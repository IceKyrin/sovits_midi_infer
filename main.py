...
# 此版本要求numpy<=1.19.2
# 此版本如使用cuda加速，自行安装cuda、cudnn等，需6g显存以上（脚本自动检测是否支持cuda）
...
cut_time = 15
model_name = "697_epochs"  # 模型名称（pth文件夹下）
...
for clean_name, bgm_name, tran in zip(clean_names, bgm_names, trans):
    ...
    for speaker_id in id_list:
        ...
        for file_name in file_list:
            ...
            pitch = icassp2022_vocal_transcription.transcribe(source_path)
            pitch[pitch != 0] += tran
            if tran == 100:
                pitch[:] = 0
            ...
            with torch.no_grad():
                ...
                audio = \
                    net_g_ms.infer(x_tst, x_tst_lengths, pitch, sid=sid, noise_scale=.1, noise_scale_w=0.5,
                                   length_scale=1)[0][
